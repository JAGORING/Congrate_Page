...
export default function ViewBucketListModal({ isOpen, onClose, item }: ViewBucketListModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="린댕이랑 준댕이가 해야할 것💌">
      <div className="bg-pink-50 p-4 rounded-lg border border-pink-200">
        <p className="text-gray-800 text-center">{item.text}</p>
      </div>
      <button
        onClick={onClose}
        className="w-full mt-4 py-2 px-4 bg-pink-400 text-white rounded-lg hover:bg-pink-500 transition"
      >
        닫기
      </button>
    </Modal>
